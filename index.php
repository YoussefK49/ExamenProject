...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['action'])) {
    ...
    if ($action === 'like' && $postId > 0) {
        addLike($postId, $defaultUserId);
        header('Location: index.php');
        exit;
    }

    if ($action === 'comment' && $postId > 0 && !empty(trim($_POST['comment_text']))) {
        addComment($postId, $defaultUserId, trim($_POST['comment_text']));
        header('Location: index.php');
        exit;
    }

    if ($action === 'create_post' && !empty(trim($_POST['caption']))) {
        createPost($defaultUserId, trim($_POST['caption']));
        header('Location: index.php');
        exit;
    }

    if ($action === 'create_story') {
        addStory($defaultUserId);
        header('Location: index.php');
        exit;
    }
}
    ...
    <aside class="sidebar">
      <div class="sidebar-profile">
        ...
        <div class="sidebar-info">
          <span class="sidebar-username">jouw_naam</span>
          <span class="sidebar-fullname">Jouw Naam</span>
        </div>
        <button class="switch-btn">Wisselen</button>
      </div>
